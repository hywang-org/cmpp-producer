...
import com.zx.sms.handler.cmpp.CMPPSubmitLongMessageExtractContentHandler;
...
public class CMPPClientEndpointConnector extends AbstractClientEndpointConnector {
	...
	protected void doBindHandler(ChannelPipeline pipe, EndpointEntity cmppentity) {
		...
		if (entity instanceof ClientEndpoint) {
			pipe.addAfter(GlobalConstance.codecName, "reWriteSubmitMsgSrcHandler",
					new ReWriteSubmitMsgSrcHandler(entity));
		}
		// 处理长短信
		// pipe.addLast("CMPPDeliverLongMessageHandler", new
		// CMPPDeliverLongMessageHandler(entity));
		// pipe.addLast("CMPPSubmitLongMessageHandler", new
		// CMPPSubmitLongMessageHandler(entity));
		pipe.addLast("CMPPSubmitLongMessageExtractContentHandler",
				new CMPPSubmitLongMessageExtractContentHandler(entity));
	...
	protected void doinitPipeLine(ChannelPipeline pipeline) {
		...
		if (entity instanceof CMPPEndpointEntity) {
			pipeline.addLast(GlobalConstance.IdleCheckerHandlerName, new IdleStateHandler(0, 0,
					((CMPPEndpointEntity) getEndpointEntity()).getIdleTimeSec(), TimeUnit.SECONDS));
