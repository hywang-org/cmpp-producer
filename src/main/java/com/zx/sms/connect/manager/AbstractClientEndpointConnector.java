...
public abstract class AbstractClientEndpointConnector extends AbstractEndpointConnector {
    ...
    public AbstractClientEndpointConnector(EndpointEntity endpoint) {
        super(endpoint);
        bootstrap.group(EventLoopGroupFactory.INS.getWorker()).channel(NioSocketChannel.class)
                .option(ChannelOption.TCP_NODELAY, true).option(ChannelOption.SO_RCVBUF, 2048)
                .option(ChannelOption.SO_SNDBUF, 2048).option(ChannelOption.ALLOCATOR, PooledByteBufAllocator.DEFAULT)
            ...
            public void operationComplete(ChannelFuture f) throws Exception {
                if (!f.isSuccess()) {
                    if (idx + 1 < hosts.length) {
                        ...
                        logger.error("Connect to {}:{} failed. cause by {}.", getEndpointEntity().getHost(), port,
                                f.cause().getMessage());
                    }
                }
            }
