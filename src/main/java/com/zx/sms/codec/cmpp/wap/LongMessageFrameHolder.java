...
public enum LongMessageFrameHolder {
	...
	public static ConcurrentHashMap<String, List<LongSMSMessage>> msgHashMap = new ConcurrentHashMap<String, List<LongSMSMessage>>();
		...
		public void onRemoval(RemovalNotification<String, FrameHolder> notification) {
			...
			case COLLECTED:
				logger.error("Long Message Lost cause by {}. {}|{}|{}|{}", cause,
						DateFormatUtils.format(h.getTimestamp(), DateFormatUtils.ISO_DATETIME_FORMAT.getPattern()),
						notification.getKey(), h.getSequence(),
	 ...
	 */
	private static Cache<String, FrameHolder> cache = CacheBuilder.newBuilder().expireAfterAccess(2, TimeUnit.HOURS)
	...
	private SmsMessage generatorSmsMessage(FrameHolder fh, LongMessageFrame frame) throws NotSupportedException {
		...
		if (udheader == null) {
			...
			if (SmsUdhIei.APP_PORT_16BIT.equals(udheader.udhIei)) {
				...
				logger.warn("UnsupportedportMessage UDH:0x{} udhdata:{},pdu:[{}]",
				...
				// 其它都当成文本短信
				logger.warn("Unsupported UDH:0x{} udhdata:{},pdu:[{}]",
						ByteBufUtil.hexDump(new byte[] { udheader.udhIei.getValue() }),
						ByteBufUtil.hexDump(udheader.infoEleData), ByteBufUtil.hexDump(contents));
				return buildTextMessage(contents, frame.getMsgfmt());
			}
