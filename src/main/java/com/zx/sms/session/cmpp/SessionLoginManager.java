...
import com.i.server.data.mysql.entity.ProSecret;
...
public class SessionLoginManager extends AbstractSessionLoginManager {
	...
	private int validClientMsg(CmppConnectRequestMessage message) throws Exception {
		...
		if (password == null) {
			...
			// System.out.println("proSecret appid = " + proSecret.getAppId());
			password = proSecret.getAppSecret();
			// redisOperationSetsMap.get(RedisConsts.REDIS_APP_INFO).
		}

		byte[] userBytes = message.getSourceAddr().getBytes(Charset.forName("utf-8"));
		System.out.println("password = " + password);
		System.out.println("thread = " + Thread.currentThread().getName());
		byte[] passwdBytes = password.getBytes(Charset.forName("utf-8"));

		byte[] timestampBytes = String.format("%010d", message.getTimestamp()).getBytes(Charset.forName("utf-8"));
		...
		if (Arrays.equals(authBytes, message.getAuthenticatorSource())) {
			...
			logger.error("AuthenticatorSource valided failed.s:{},c:{}", Hex.encodeHexString(authBytes),
					Hex.encodeHexString(message.getAuthenticatorSource()));
			return 3;
		}
	}

	private int validClientMsg(CmppConnectRequestMessage message, CMPPServerChildEndpointEntity entity)
			throws Exception {
		// return 0;
		System.out.println("cliententity.getRedisOperationSets().get(1) =" + entity.getRedisOperationSets().get("1"));
	...
	protected void changeProtoVersion(ChannelHandlerContext ctx, EndpointEntity entity, Object msg) throws Exception {
		...
		if ((short) 0x30 != childentity.getVersion()) {
			...
			if (childentity.getVersion() != version) {
				logger.warn("receive version code {} ,expected version is {} .I would use version {}", version,
						childentity.getVersion(), childentity.getVersion());
			}
	...
	protected void doLoginSuccess(ChannelHandlerContext ctx, EndpointEntity entity, Object msg) {
		...
		resp.setStatus(0);
		resp.setAuthenticatorISMG(DigestUtils.md5(Bytes.concat(Ints.toByteArray((int) resp.getStatus()),
				message.getAuthenticatorSource(), childentity.getPassword().getBytes(childentity.getChartset()))));
	...
	// ly modify
	@Override
	protected int validClientMsg2(Object message) {
		try {
			return validClientMsg((CmppConnectRequestMessage) message);
		} catch (Exception e) {
			logger.error("AuthenticatorSource valided failed", e);
			return 3;
		}
