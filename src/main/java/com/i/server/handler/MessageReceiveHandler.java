...
public abstract class MessageReceiveHandler extends AbstractBusinessHandler {
	...
	public synchronized void userEventTriggered(final ChannelHandlerContext ctx, Object evt) throws Exception {
		if (evt == SessionState.Connect && !inited) {
			// EventLoopGroupFactory.INS.submitUnlimitCircleTask(new
			// Callable<Boolean>() {
			//
			// @Override
			...
			//
			// long nowcnt = cnt.get();
			// logger.info("channels : {},Totle Receive Msg Num:{}, speed :
			// {}/s",
			// EndpointManager.INS.getEndpointConnector(getEndpointEntity()).getConnectionNum(),
			// nowcnt,
			// (nowcnt - lastNum) / rate);
			// lastNum = nowcnt;
			// return true;
